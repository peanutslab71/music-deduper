# Distributing Music Deduper (public, no Gatekeeper warnings)
...
3. Open the project → select the **MusicDeduper** target → **Signing & Capabilities**:
...
1. Set the run destination to **Any Mac** (top toolbar), scheme **MusicDeduper**, Release.
...
4. Xcode signs it, uploads it to Apple for notarization, and (after a few minutes) marks it
   **Ready to distribute**. Click **Export** and save `MusicDeduper.app` to a folder.
...
```bash
chmod +x make_dmg.sh
./make_dmg.sh "/path/to/exported/MusicDeduper.app"
```

You'll get **`Music Deduper.dmg`** with a drag-to-Applications layout.
...
```bash
xcrun notarytool submit "Music Deduper.dmg" --keychain-profile "MD_NOTARY" --wait
xcrun stapler staple "Music Deduper.dmg"
```
...
```bash
spctl -a -vvv -t open --context context:primary-signature "Music Deduper.dmg"   # should say: accepted, source=Notarized Developer ID
xcrun stapler validate "Music Deduper.dmg"                                        # should say: The validate action worked
```

Now anyone can download `Music Deduper.dmg`, open it, drag the app to Applications,
