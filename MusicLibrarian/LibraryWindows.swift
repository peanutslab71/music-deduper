...
enum AlbumPerfect {
    ...
    /// `reconcileOnline: false` skips the MusicBrainz release lookup (the
    /// missing-tracks / disc-correction reconcile) — the user's persisted
    /// "check for missing tracks" gate, so a whole-library run can stay fast.
    static func analyze(root: URL, files: [URL], reconciledMatch: MBReleaseMatch? = nil,
                        runIdentify: Bool = true,
                        reconcileOnline: Bool = true) async -> (fixes: [AlbumFix], art: AlbumArtContext, reconcile: MBReleaseMatch?) {
        ...
        if let cached = reconciledMatch {
            match = cached
        } else if reconcileOnline, kept.count >= 4 {
