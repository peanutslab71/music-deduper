...
#   2. Writes /etc/nsmb.conf with two settings:
#        signing_required=no   — stop cryptographically signing every packet
#                                (big per-packet overhead, pointless on a guest
#                                share on your own network)
